...
df_et_ribbon_def <- df_et_ribbon %>% 
  dplyr::filter(hist > `45`, wyd>200)
df_et_ribbon_pos1 <- df_et_ribbon %>% 
  dplyr::filter(hist < `45`, wyd<270)
df_et_ribbon_pos2 <- df_et_ribbon %>% 
  dplyr::filter(hist < `45`, wyd>280)
...
df_q_ribbon_def <- df_q_ribbon %>% 
  dplyr::filter(hist > `45`, wyd>100)
df_q_ribbon_pos <- df_q_ribbon %>% 
  dplyr::filter(hist < `45`, wyd<300)
...
# ET
x <- ggplot() +
  geom_ribbon(data=df_et_ribbon_def, aes(x=date, ymin=`45`, ymax=hist), fill="firebrick2") +
  geom_ribbon(data=df_et_ribbon_pos1, aes(x=date, ymin=hist, ymax=`45`), fill="dodgerblue") +
  geom_ribbon(data=df_et_ribbon_pos2, aes(x=date, ymin=hist, ymax=`45`), fill="dodgerblue") +
  geom_line(data=df_line, aes(x=date, y=ET, color=scenario)) +
  scale_x_date(labels = date_format("%b")) +
  scale_color_brewer(palette="Paired", name="Scenario", labels = c("Historical","4.5")) +
  labs(title = "Mean Daily ET", x = "Month", y = "ET (mm)") +
  facet_grid(watershed~., labeller = as_labeller(shed_id)) +
  theme_bw(base_size = 13) +
  NULL
plot(x)
ggsave("output/mean_daily_et_ribbon_45.jpg", width = 6, height = 5)


# Q
x <- ggplot() +
  geom_ribbon(data=df_q_ribbon_def, aes(x=date, ymin=`45`, ymax=hist), fill="firebrick2") +
  geom_ribbon(data=df_q_ribbon_pos, aes(x=date, ymin=hist, ymax=`45`), fill="dodgerblue2") +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_q_ribbon_45.jpg", width = 6, height = 5)


# SWE
x <- ggplot() +
  geom_ribbon(data=df_swe_ribbon_def, aes(x=date, ymin=`45`, ymax=hist), fill="red") +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_swe_ribbon_45.jpg", width = 6, height = 5)


# SM
x <- ggplot() +
  geom_ribbon(data=df_sm_ribbon_def, aes(x=date, ymin=`45`, ymax=hist), fill="firebrick2") +
  geom_ribbon(data=df_sm_ribbon_pos, aes(x=date, ymin=hist, ymax=`45`), fill="dodgerblue2") +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_sm_ribbon_45.jpg", width = 6, height = 5)
