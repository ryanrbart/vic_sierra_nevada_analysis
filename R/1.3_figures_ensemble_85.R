...
df_et_ribbon_pos1 <- df_et_ribbon %>% 
  dplyr::filter(hist < `85`, wyd<270)
df_et_ribbon_pos2 <- df_et_ribbon %>% 
  dplyr::filter(hist < `85`, wyd>280)
...
x <- ggplot() +
  ...
  geom_ribbon(data=df_et_ribbon_pos1, aes(x=date, ymin=hist, ymax=`85`), fill="dodgerblue") +
  geom_ribbon(data=df_et_ribbon_pos2, aes(x=date, ymin=hist, ymax=`85`), fill="dodgerblue") +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_et_ribbon_85.jpg", width = 6, height = 5)
...
x <- ggplot() +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_q_ribbon_85.jpg", width = 6, height = 5)
...
x <- ggplot() +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_swe_ribbon_85.jpg", width = 6, height = 5)


# SM
x <- ggplot() +
  geom_ribbon(data=df_sm_ribbon_def, aes(x=date, ymin=`85`, ymax=hist), fill="firebrick2") +
  geom_ribbon(data=df_sm_ribbon_pos, aes(x=date, ymin=hist, ymax=`85`), fill="dodgerblue2") +
  ...
  NULL
plot(x)
ggsave("output/mean_daily_sm_ribbon_85.jpg", width = 6, height = 5)
